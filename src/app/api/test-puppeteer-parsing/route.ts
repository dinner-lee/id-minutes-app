...
import { fetchChatGPTSharePuppeteer, debugChatGPTSharePuppeteer } from "@/lib/chatgpt-ingest";
...
export async function POST(req: NextRequest) {
  try {
    ...
    console.log(`Testing Puppeteer ChatGPT parsing for URL: ${url}`);
    
    if (debug) {
      // Use debug function for detailed analysis
      const debugInfo = await debugChatGPTSharePuppeteer(url);
    ...
    return NextResponse.json({ 
      ok: false, 
      error: e?.message || "Puppeteer parsing failed",
      stack: e?.stack
    }, { status: 400 });
  }
}
