// src/lib/chatgpt-playwright.ts
import { SharePayload } from './chatgpt-ingest';

/** ChatGPT parsing using Playwright (reliable for Vercel) */
export async function fetchChatGPTSharePlaywright(url: string): Promise<SharePayload> {
  ...
  console.log("Using Playwright for ChatGPT parsing...");
  ...
  try {
    ...
    console.error("Playwright parsing failed:", error);
    ...
    if (error.message?.includes('timeout')) {
      ...
      throw new Error(`Playwright parsing failed: ${error.message || 'Unknown error'}`);
    }
  }
}
